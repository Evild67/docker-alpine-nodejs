...
RUN \
    build_pkgs="git curl make gcc g++ python linux-headers libgcc libstdc++ binutils-gold xz" \
    && apk --no-cache add {build_pkgs}  \
    && curl -SLO https://nodejs.org/dist/${NODE_VERSION}/node-v${NODE_VERSION}.tar.xz\
